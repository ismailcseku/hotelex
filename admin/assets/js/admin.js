(function ($) {
  ...
  $body
    .on("click", ".tm-widget-image-upload", function (event) {
      ...
      file_frame[frame_key].on("select update insert", function (e) {
        var selection,
          state = file_frame[frame_key].state();
  ...
  $body
    .on("click", ".mascot-megamenu-upload-bgimage", function (event) {
      event.preventDefault();
  ...
  if ($(".mascot-menuicon-picker").length > 0) {
