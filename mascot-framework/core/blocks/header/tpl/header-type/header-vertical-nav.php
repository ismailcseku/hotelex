	...
	<?php do_action( 'hotelex_before_header_nav' ); ?>
			...
			<div class="menuzord-container header-nav-container <?php echo esc_attr( $header_nav_row_bg_theme_colored ); ?>">
				<div class="<?php echo esc_attr( $header_layout_container_class ); ?> position-relative">
					<div class="row">
						...
						<?php
							/**
							* hotelex_header_nav_container_start hook.
							*
							*/
							do_action( 'hotelex_header_nav_container_start' );
						?>
						...
						<nav id="top-primary-nav" class="menuzord-primary-nav menuzord menuzord-responsive menuzord-responsive <?php echo esc_attr( $navigation_color_scheme );?>" data-effect="<?php echo esc_attr( $navigation_primary_effect );?>" data-animation="<?php echo esc_attr( $navigation_css3_animation );?>" data-align="right">
							...
							<?php
								/**
								* hotelex_header_logo hook.
								*
								* @hooked hotelex_get_header_logo
								*/
								do_action( 'hotelex_header_logo' );
							?>

							<?php
								/**
								* hotelex_header_primary_nav hook.
								*
								* @hooked hotelex_get_header_primary_nav
								*/
								do_action( 'hotelex_header_primary_nav', 'main-nav' );
							?>
						...
						<?php else: ?>
							<h4><?php esc_html_e( 'This is your Vertical Nav Sidebar!', 'hotelex' )?></h4>
							<p><?php echo sprintf( esc_html__( 'Please add your sidebar widgets to this section from %1$sAppearance > Widgets%2$s (Vertical Nav Sidebar).', 'hotelex' ), '<strong>', '</strong>')?></p>
						<?php endif; ?>
						</div>

						<?php
							/**
							* hotelex_header_nav_container_end hook.
							*
							*/
							do_action( 'hotelex_header_nav_container_end' );
						?>
					...
					<div class="row top-primary-nav-clone-parent">
						<div class="col-12">
							...
							<?php
								...
								* @hooked hotelex_get_header_primary_nav
								*/
								do_action( 'hotelex_header_primary_nav', 'main-nav-clone' );
							?>
							</nav>
						</div>
					</div>
	...
	<?php do_action( 'hotelex_after_header_nav' ); ?>
	<?php endif; ?>