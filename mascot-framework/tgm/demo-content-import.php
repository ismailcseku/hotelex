...
class HotelexOneClickImport {
	...
	function __construct() {
		...
		$this->import_sliders = true;

		//FTP
		$this->revSliderFolder_importURI = 'https://kodesolution.live/one-click-demo-import/hotelex/import-rev-sliders/';
		$this->base_url = 'https://kodesolution.live/one-click-demo-import/hotelex/v1.0/';

		//HTTP
		$this->preview_base_url = 'https://dev251.kodesolution.com/hotelex/';
	...
	public function rev_sliders($folder) {
		$slider_variations = array (
			'home1' => 'Slider-Home1.zip,',
			'rtl' => 'Slider-Home1.zip'
		);
			...
			$final_output[] = array(
					...
					'import_redux'               => array(
							array(
									...
									'option_name' => 'hotelex_redux_theme_opt',
							),
					),
					'import_preview_image_url'   => $current_url . 'screenshot.jpg',
					'import_notice'              => esc_html__( 'After you import this demo, you will have to import the slider separately. You will find slider in "import-sliders" folder in your downloaded package.', 'hotelex' ),
	...
	public function plugin_page_setup( $default_settings ) {
		...
		$default_settings['page_title']  = esc_html__( 'One Click Demo Import - ThemeMascot' , 'hotelex' );
		$default_settings['menu_title']  = esc_html__( 'Import Demo Data' , 'hotelex' );
					...
					foreach ( $demo_rev_sliders as $demo_rev_slider ) {
						...
						if ( ! in_array( str_replace( '.zip', '', $demo_rev_slider ), $rev_sliders, true ) ) {
							...
							if ( ! $rev_slider_result['success'] ) {
								esc_html_e( 'Error while importing rev sliders', 'hotelex' );
								exit;
							}
						}
					}

					esc_html_e( 'Rev sliders imported successfully', 'hotelex' );
				} else {
					esc_html_e( 'RevSliderSliderImport instance is empty', 'hotelex' );
				}

			} else {
				esc_html_e( 'RevSliderSliderImport class doesn\'t exist', 'hotelex' );
			}

		} else {
			esc_html_e( 'Files don\'t exist', 'hotelex' );
		}
	}

}

HotelexOneClickImport::get_instance();